...
export async function getAllAdminPayments(
  ...
  try {
    console.log('🔍 Debug: Fetching admin payments...');
    ...
    console.log('📊 Debug: Total payments count:', totalCount, 'Error:', countError?.message);
...
export async function createUnlockRequest(
  ...
  try {
    ...
    console.log('🔍 Creating unlock request:', { postId, userId, visitorId });
    
    // Create a unique request ID that we can use to identify the request
    ...
    console.log('🔍 Unlock request result:', { data, error: error?.message });
    ...
    // If ID is null or undefined, use visitor_id as the request identifier
    const requestId = data?.id || visitorId;
    
    if (!requestId) {
      console.error('❌ No ID or visitor_id available');
      return { success: false, error: 'Failed to get request identifier' };
    }

    console.log('✅ Unlock request created with identifier:', requestId);
