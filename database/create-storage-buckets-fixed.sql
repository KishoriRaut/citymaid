-- Create missing storage buckets for production (FIXED VERSION)
...
-- Create policies for payment-proofs bucket (using users table instead of profiles)
CREATE POLICY "Admins can view payment proofs" ON storage.objects
FOR SELECT USING (
    bucket_id = 'payment-proofs' AND
    EXISTS (
        SELECT 1 FROM public.users 
        WHERE users.email = 'kishoriraut369@gmail.com'
    )
);
