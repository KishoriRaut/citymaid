...
-- There are two versions of get_public_posts function causing PGRST203 error

-- Drop both versions to clean up (using correct PostgreSQL syntax)
DROP FUNCTION IF EXISTS public.get_public_posts();
DROP FUNCTION IF EXISTS public.get_public_posts(text, text, text, integer, integer, text);

-- Create the simple version without parameters
