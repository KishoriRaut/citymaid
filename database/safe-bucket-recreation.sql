-- Safe Bucket Recreation - Clean existing objects first
...
-- Step 2: Delete all objects from payment-proofs bucket first (to avoid foreign key constraint)
DELETE FROM storage.objects WHERE bucket_id = (
    SELECT id FROM storage.buckets WHERE name = 'payment-proofs'
);

-- Step 3: Now safely delete the bucket
DELETE FROM storage.buckets WHERE name = 'payment-proofs';

-- Step 4: Create payment-proofs bucket with proper settings
...
-- Step 6: Drop all existing policies for this bucket
...
-- Step 11: Generate test URL format
SELECT 
...
SELECT 
    'Status' as step,
    'Payment-proofs bucket recreated successfully!' as message,
    NOW() as completed_at;
