...
export default function RequestsPage() {
  ...
  const viewPaymentProof = (paymentProof: string | null) => {
    ...
    console.log('🔍 Viewing payment proof:', paymentProof);
    console.log('🔍 Payment proof starts with http:', paymentProof.startsWith('http'));
    ...
    } else if (paymentProof.includes('storage/v1/object/public/')) {
      // If it's a relative storage path, construct full URL
      const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
      receiptUrl = paymentProof.startsWith('/') 
        ? `${supabaseUrl}${paymentProof}`
        : `${supabaseUrl}/${paymentProof}`;
      console.log('🔍 Constructed URL from storage path');
    } else {
      // Use our API endpoint to serve the file
      receiptUrl = `/api/payment-receipts/${paymentProof}`;
